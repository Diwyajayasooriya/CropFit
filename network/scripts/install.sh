...
# automatically — see pairing_watcher.py).
set -euo pipefail
...
echo "== 2/14: copy repo to $INSTALL_DIR =="
mkdir -p "$INSTALL_DIR"
cp -r "$REPO_DIR"/* "$INSTALL_DIR"/

echo "== 3/14: udev rule (stable wlan1 naming for the USB adapter) =="
cp "$REPO_DIR/config/udev/70-greennode-net.rules" /etc/udev/rules.d/
udevadm control --reload-rules

echo "== 4/14: hostapd + wpa_supplicant configs =="
...
echo "== 5/14: static IPs for AP interfaces (dhcpcd) =="
...
echo "== 10/14: deploy edge application layer (ingestion, dispatcher, rules) =="
if [[ -n "$EDGE_DIR" ]]; then
...
echo "== 13/14: ingestion service + rules engine timer =="
...
echo "== 14/14: NTP (clock sync — matters for reading timestamps on cold boot) =="
...
echo "  systemctl status hostapd dnsmasq mosquitto greennode-tc greennode-shedding \\"
echo "    greennode-pairing-watcher greennode-ingestion greennode-rules.timer"
